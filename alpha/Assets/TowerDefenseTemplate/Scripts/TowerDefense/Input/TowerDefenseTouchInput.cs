...
namespace TowerDefense.Input
{
	...
	public class TowerDefenseTouchInput : TouchInput
	{
		...
		protected override void OnPress(PointerActionInfo pointer)
		{
			...
			if (touchInfo != null)
			{
				if (m_GameUI.state == GameUI.State.Building)
				{
		...
		protected virtual void OnTap(PointerActionInfo pointerActionInfo)
		{
			...
			if (touchInfo != null)
			{
				if (m_GameUI.state == GameUI.State.Normal && !touchInfo.startedOverUI)
				{
					m_GameUI.TrySelectTower(touchInfo);
				}
				else if (m_GameUI.state == GameUI.State.Building && !touchInfo.startedOverUI)
				{
		...
		protected override void OnDrag(PointerActionInfo pointer)
		{
			...
			if (touchInfo != null)
			{
				...
				if (m_GameUI.state == GameUI.State.BuildingWithDrag)
				{
					DragGhost(touchInfo);
				}
				else
		...
		/// </param>
		void OnStateChanged(GameUI.State previousState, GameUI.State currentState)
		{
			...
			if (currentState == GameUI.State.Building && previousState != GameUI.State.BuildingWithDrag)
			{
				m_GameUI.MoveGhostToCenter();
				confirmationButtons.canvasEnabled = false;
				invalidButtons.canvasEnabled = false;
			}
			if (currentState == GameUI.State.BuildingWithDrag)
			{
